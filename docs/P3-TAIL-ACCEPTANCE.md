...
# FINAL UNIT iteration 2 (2026-07-19, after the U1 resume-3 verbatim `neutral`): premise-scoped entailment + a direct-breach second control

U1 resume 3 proved the door fixed the polarity (contradiction became neutral) and the model again ruled `breach` correctly; Gate 3 now correctly rules `neutral` because the quote alone ("wrinkle-relaxing injections") cannot prove "prescription only medicine". The bridge is not model knowledge: it is the record's OWN duty clause listing indirect references verbatim. Constitution scoping: the LLM judges inside the catalogue's closed world (Rule 11), and the catalogue is the trusted source of law facts (Rule 2), so the NLI premise must be the FULL trusted evidence set: the verbatim quote PLUS the record's own verbatim rule text as a second, DOC-delimited, catalogue-sourced premise. The hypothesis stays the atomic claim; `contradiction` and `neutral` still demote; the model's own world knowledge remains excluded. This validates composition, never re-derivation, and is not a loosening: a compliant quote plus the same bridge still entails nothing.

## Builder B resume 3. Owns llm/entailment.js (premise assembly + prompt), the gateEntailment/claimFor seam in breach/adjudicator/adjudicate.js, breach/adjudicator/claim.js (only if the bridge extraction belongs beside the claim derivation), llm/evals/ additions, plus tests. Does not touch fixtures, proposers, run-real-proof.js, recordings.

- G1: the entailment call's premise set gains a second document: the owning record's relevant duty text (the clause carrying the prohibition and its indirect-reference examples), verbatim from the compiled catalogue, DOC-delimited through the sanitisation door, clearly labelled as rule text distinct from page evidence. Deterministic extraction (duty_idx), no authored content.
- G2: the entailment prompt states the premises comprise page evidence AND the rule's own text, and that the hypothesis must follow from the premises alone. Prompt change discipline (Rule 11): extend llm/evals fixtures with the new shapes (indirect-with-bridge entails; compliant-with-bridge does not; no-bridge indirect stays neutral) and keep the precision and abstain gates green.
- G3: tests both directions, hermetic: (a) scripted entailment affirms an indirect-reference candidate when the bridge premise is present; (b) a compliant premise with the same bridge does NOT entail (C-048); (c) the bridge text is provably catalogue-verbatim; (d) contradiction and neutral still demote through the real adjudicator.
- Benchmarks: full suite green (1270+); sweep GREEN ACT 0; calibration --strict OK; llm-evals precision 1.0 and abstain gates green; zero new health findings; scripted canonical still exits 1 with ONE result line; red-team 9 entries 0 escapes.

## Builder A resume: a second, DIRECT-breach synthetic control. Owns eval/e2e fixture additions and their expectations only.

- H1: inspect the compiled specs for one whose matcher includes a phrase that DIRECTLY states the prohibited thing (no domain bridge needed). If one exists, add a second synthetic fixture (1 page, same self-documenting style) planting that phrase verbatim, with its own known_breach token; wire it into the harness synthetic lane beside the existing control. If NO compiled spec supports a direct phrasing honestly, STOP and report (catalogue additions are founder-gated; do not bend an absence spec into a presence control, C-048).
- Benchmarks: the new control proposes, verifies and reaches adjudication in scripted mode (demoting on decline, correct); suite, sweep, calibration all green; zero new health findings.

## U1 resume 4 (after both land): re-record everything (premise change invalidates prior raw responses even though keys are unchanged), expect entailment on the indirect control (and the direct control if added), then the canonical replay run must show reproduced >= 1 and exit 0. The honesty rule is absolute and final for this wave: if the live model still refuses, the wave closes honestly RED on the vacuity bar, the PR does not open, and the residual routes to the next session with the verbatim verdicts on record.

---

# FINAL UNIT iteration 3 (2026-07-19, after U1 resume 4): bridge-as-glossary

U1 resume 4 (committed) proved the ENGINE reasons correctly end to end: with the bridge live the model resolved the indirect reference and its rationale reads "the website does advertise such medicine to the public" - correct - yet it labelled the relation `contradiction` and misquoted the affirmative hypothesis ("This website does advertise...") as "the hypothesis that it does not". Verbatim Gate-3 verdict on record. Diagnosis: the bridge is the record's FULL duty, which OPENS with the deontic prohibition "Do not advertise..."; that operator is the first rule-text the model reads and primes a hypothesis-polarity inversion. The entailment premise should carry the record's DEFINITIONAL/FACTUAL mapping (what terms count as the thing), never the prohibition (the prohibition is adjudication's job, already done correctly). This is an architectural correction, not a loosening.

## Builder B resume 4. Owns breach/adjudicator/claim.js (bridgeTextFor), llm/prompts/entailment.js (only if the glossary framing needs a prompt tweak), llm/evals additions, tests. Does not touch fixtures, proposers, run-real-proof.js, recordings.

- I1: bridgeTextFor derives a GLOSSARY premise, not the full duty: extract only the definitional/enumerative content (the indirect-reference listing, e.g. "wrinkle-relaxing injections and fat jab are references to prescription only medicines") and, if the duty does not cleanly separate, construct a neutral definitional framing from the enumerated terms WITHOUT any imperative/prohibition operator ("do not", "must not", "may not", "prohibited"). Deterministic, catalogue-verbatim terms, no authored law facts (Rule 2/11). If a record carries no enumerable definitional content, bridgeTextFor returns '' (no bridge, single-premise path, the pre-iteration-2 behaviour).
- I2: prove the operator is gone: a test asserts the bridge for UK_MHRA_POM_AD_BAN contains "wrinkle-relaxing injections" AND contains no prohibition operator token.
- I3: both-direction llm-evals unchanged in intent and still green (indirect page + glossary bridge classes entailment under a faithful scripted model; compliant page + same glossary classes neutral, the C-048 direction; no-bridge indirect stays neutral). Update the three ent-bridge fixtures' premise text to the glossary shape.
- Benchmarks: full suite green (1290+); sweep GREEN ACT 0; calibration --strict OK; llm-evals precision 1.000; zero new health findings; scripted canonical still exits 1 with ONE result line; red-team 9 entries 0 escapes.

## U1 resume 5 (after B): re-record the synthetic + 4 forensic firms with the engine's own chain under the glossary bridge. If Gate 3 now labels entailment, the canonical replay shows reproduced >= 1 and exit 0 - the wave's green. Honesty rule absolute: if the single model still mislabels, capture the verbatim verdict and the wave closes RED at the honest frontier (single-model NLI label reliability), whose robust fix is the Gate-5 diverse jury (a deferred unit, needs the independent model family the OpenRouter/Nemotron evaluation identifies). Do NOT cherry-pick: a single entailment on one run is reported as a single-run result, not declared reliable; if variance persists, that IS the finding and it routes to the jury unit.

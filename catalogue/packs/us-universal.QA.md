<!-- qa-approval pack_sha256=c75c7e0ac3b357ef82922c618cfe271e13088828e4c89bebab457cdef1960002 verdict=approved reviewed=2026-07-28 -->
...
## Scope-gate wave (2026-07-28): `requires_activity` added to US_COPPA and US_BIPA_IL
New applicability **gate 5b** (fail-closed scope predicate, `applicability/connect.js#gateRequiredActivity`). PROVEN DEFECT this closes: a live dry-run mint of a US immigration law firm produced **5 CONFIRMED US_COPPA "violations"** off nothing but pre-consent Google Analytics cookies (`_ga`, `_gid`), carrying a children's-privacy statute (typical_high $275m) into the exposure of a firm with no child-directed content whatsoever. Gate 5 is deliberately fail-OPEN (unknown never excludes), which is correct for incidental tags (`b2c`/`cookies_present`/`runs_ads`) but wrong where the capability IS the statute's scope - and `child_directed` has positive-only detection, so it can never be affirmatively disproven and COPPA could never be excluded.
- `US_COPPA` -> `requires_activity: ["child_directed"]` (applies_when: "the site or service is directed to children under 13, or the operator has actual knowledge it collects personal information from a child under 13").
- `US_BIPA_IL` -> `requires_activity: ["biometrics"]` (applies_when: "collects biometric identifiers ... face/voice/fingerprint geometry").
No law fact, citation, penalty or obligation changed; this is a binding-correctness fix that removes a false-accusation class.

<!-- qa-approval pack_sha256=e3fb9f5511d4c4a12b69dbf2d199387c067076cdac732d4f6395e39664f0d2b2 verdict=approved reviewed=2026-07-27 -->
...
Two records added, each verified against the official source on 2026-07-27; no existing record touched:
- **NEW RECORD `UK_NMC_CODE_ADVERTISING`** - NMC Code paras verified verbatim at nmc.org.uk: 21.4 (advertisements "accurate, responsible, ethical, do not mislead or exploit vulnerabilities and accurately reflect your relevant skills, experience and qualifications"), 21.5 ("never use your status as a registered professional to promote causes that are not related to health"), 20 (uphold the reputation of the profession). Closes the nurse-led/aesthetic-nurse gap (GMC/GDC records do not cover NMC registrants). One register-evidence duty (claimed NMC registration corroborated on the public register - register_url search-the-register) + one behavioural advertising duty. Penalty basis is honest: NMC FtP outcomes, no fines; ASA/CAP record handles ad-fine exposure separately.
- **NEW RECORD `UK_GDPR_ART9_HEALTH_TRANSPARENCY`** - UK GDPR Art 9(2)(h) + DPA 2018 s.10 + Sch 1 Part 1 para 2 (health/social care condition) + Arts 13-14 transparency. DELIBERATE FRAMING GUARD: ICO guidance confirms the para-2 health-care condition does NOT require an appropriate policy document, so the duty is authored as privacy-notice transparency (name the special-category processing + the Art 9 condition relied on, evidence_type presence) and never as a missing-APD accusation - an APD accusation against a para-2 processor would be a FALSE accusation. Penalty mirrors the uk-universal GDPR higher-maximum convention (17.5m/4%, typical band from 2025 ICO penalties, penalty_model turnover_pct).
Enforcement depth for these two records is deliberately empty rather than padded: no specific ASA/ICO/CQC case was re-verified at source this session. - Linter-driven correction (same wave): `UK_NMC_CODE_ADVERTISING` sub_sector trimmed to the six specific leaves (the 'healthcare' family tag is not a CANONICAL_SUB_SECTORS member; the leaves alone carry the binding). 'nmc.org.uk' added to the citation linter's OFFICIAL_HOSTS (the NMC is the statutory regulator under the Nursing and Midwifery Order 2001, same class as gmc-uk.org/gdc-uk.org).

...
## Orchestrator decision: P6 connection-integrity (build agent, 2026-07-19)

**Defect found (not a legal defect - a binding defect, the empirical-healthcare D4 class extended to the barrister population).** `UK_BSB_HANDBOOK_PUBLICITY` and `UK_BSB_TRANSPARENCY` both carry `sector: ["legal"]`. `facts/vocabulary.js SECTOR_ALIASES` folds the string `"legal"` to the canonical top-level sector key `"law-firms"` (there is no canonical `"legal"` node). `applicability/connect.js gateSector()` therefore only matches a firm whose OWN resolved sector/family set contains `"law-firms"`. A real barrister or chambers site resolves to the SEPARATE top-level sector `"barristers"` (`facts/vocabulary.js SECTORS.barristers`, no `parent` field, so it never folds up into `law-firms`) - confirmed via `facts/sector.js resolveSector()` against a genuine chambers sample ("Ashford Chambers offers direct access to our barristers... public access instructions... instructing counsel", `facts/sector.test.js` new test "P6: a genuine barristers chambers resolves barristers/general"). So neither record could ever bind a barrister firm: gate-4's SECTOR half (not the sub-sector half - both records' `sub_sector: ["barristers", "chambers"]` was already correctly authored and would have bound fine once the sector half passed) excluded every barrister on earth, exactly the class this pack's own QA table already confirmed as legally sound and citation-verified (see `UK_BSB_HANDBOOK_PUBLICITY (confirmed)` / `UK_BSB_TRANSPARENCY (gap_filled)` rows above) but silently dead in the field.

**Fix applied.** Both records' `sector` retagged from `["legal"]` to `["barristers"]` - the canonical top-level sector key a real barrister/chambers firm actually resolves to. This is a binding-correctness fix, not a legal-substance change: the BSB Handbook and Transparency Rules obligations themselves, their citations, penalties, polarity and enforcement text are byte-for-byte unchanged from the 2026-07-17 review above. `uk-legal` now reaches 12/12 records genuinely bindable (`catalogue/linters/sub-sector-binding.js` returns 0 violations for this pack; `catalogue/linters/citation-completeness.js` and `polarity.js` remain unaffected, 0/0).

**Verification.** `facts/vocabulary.js` gained no new sub-sector detection leaves for this pack (the fix is a one-token retag, `sub_sector: ["barristers", "chambers"]` was already correct and already binds via the coarse-parent-label and `chambers` -> `general` synonym rules `facts/vocabulary.js` already carried). Regression-proven end to end: `applicability/connect.test.js` integration test "P6 wave: genuine samples bind the exact records they target" runs a synthetic barrister firm through `facts/sector.js resolveSector()` (real vocabulary) and `applicability/connect.js connect()` (the real compiled catalogue) and asserts `UK_BSB_HANDBOOK_PUBLICITY` and `UK_BSB_TRANSPARENCY` both appear in `applicable`.

**Re-stamped.** `node catalogue/compile.js --print-hashes` re-run against the corrected pack: new `pack_sha256` = `45aa7976da6d1901fac7689d8ee856b64bd7fd20720e5afdef4d10fc1fffc56c` (embedded in this sidecar's header above, `reviewed=2026-07-19`). Founder (Aman) phase sign-off at PR merge remains the one gate this document cannot itself satisfy.

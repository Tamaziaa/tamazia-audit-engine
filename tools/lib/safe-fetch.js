...
// (the safe default); an input that already carries an explicit http(s) scheme is preserved EXACTLY, never
// silently upgraded or downgraded - an operator's explicit http:// choice is honoured as given
// (playwright-adapter.js applies the one-shot http:// fallback only when THIS door defaulted the scheme,
// never on an explicit input). Playwright's page.goto() requires a URL with a scheme; without this door, a
// bare domain makes goto() THROW ("Cannot navigate to invalid URL"), and the old code path passed the raw,
// unnormalised url straight through to both browser lanes (evidence/browser/observe.js, mint/
// compose-bundle.js's domLanePipeline) while the crawl lane alone normalised its own copy (evidence/
// crawler/crawl.js). Returns '' when nothing host-shaped can be parsed (mirrors inputHost's contract; the
// caller's own deadline+catch machinery then records the empty target as a loud lane failure - never a
// silent about:blank pass) - and ALSO '' for any resolved scheme other than http:/https: (CodeRabbit PR
// #25: an operator-supplied `file:///etc/passwd`, `javascript:`, `data:` or `blob:` input - see the
// DANGEROUS_SCHEME_RX this file already refuses to crawl - must never reach a real browser's goto() and
// read/execute local or non-network content; this is the ONE door both browser lanes navigate through, so
// the scheme floor lives here once, not duplicated per lane).
function resolveNavigableUrl(raw) {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') return ''; // never file:/javascript:/data:/blob: etc.
  return parsed.href;
}

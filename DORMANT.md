...
| `evidence/browser/playwright-adapter.js` | the lazy, optional real-Playwright adapter; the only file naming Playwright, required lazily inside a function so its absence never breaks a require graph walk. T2a added an ADDITIVE `page.evaluate(fn)` delegate (used by `dom-assert.js`); every existing method is untouched | Aman | same as above; also gated on Playwright being installed in the mint runner (not exercised in CI by design, no Chromium install) |
| `evidence/browser/dom-assert.js` | the axe-style DOM assertion lane (`domAssert(page, opts)`, T2a); tested (`dom-assert.test.js`, 16 tests) with all graded decisions as pure exported predicates and the dom_node artifact it feeds calibrated downstream (`p4-verifier-fabricated-domnode.json` via `breach/verifiers/dom-node.js`, wired into `eval/calibration-known-bad/run.js`), but nothing under `mint/` calls it yet - the T1 live-mint wiring assembles the `bundle.browser.domNodes` surface this lane produces | Aman | `mint/` requires `evidence/browser/dom-assert.js` for the DOM-fact evidence step (T1 live mint) - DORMANT until then |
...
| `evidence/registers/cqc.js` | Care Quality Commission lookup; host fixed to the live `api.service.cqc.org.uk` 2026-07-19 (`api.cqc.org.uk` now 403s), `CQC_API_KEY` verified live against it, `CQC_PARTNER_CODE` made optional with a bounded HTTP-400 fallback; still cannot return a match today because the live API has no free-text name-search parameter at all (`name`/`providerName`/`search`/`q` each answer 400 "Unspecified query parameter X is not allowed", proven live 2026-07-19), a pre-existing defect this fix could not solve within its scope; consumed only by `registers.js` | Aman | same as above, plus wiring the verified `CQC_API_KEY` into ENV_B64, CQC Partner Programme enrolment for `CQC_PARTNER_CODE`, and a scoped decision on the missing name-search capability (register-lanes PR report) |
| `evidence/registers/fca.js` | Financial Conduct Authority register lookup; shape re-researched and re-verified live 2026-07-19 against official/actively-maintained sources, unchanged; this estate's on-file `FCA_API_EMAIL`/`FCA_API_KEY` still 404 on every live call (403 with no auth at all, so the key is having an effect), consistent with an account-side entitlement problem rather than a code defect; consumed only by `registers.js` | Aman | same as above, plus founder sign-in at the FCA developer portal to confirm/regenerate the API product subscription for this key (register-lanes PR report) |
...
| `breach/verifiers/index.js` | the artifact-type dispatch door (T2a): re-exports `quote-match.js`'s public API verbatim EXCEPT `verifyCandidate`/`verifyAll`, which it overrides to route a `dom_node` artifact to `dom-node.js` and delegate every other type to `quote-match.js` unchanged; tested (`index.test.js`). Was a pure re-export shim before T2a | Aman | same as above |
| `breach/verifiers/dom-node.js` | the `dom_node` artifact verifier (`verifyDomNode`, T2a): fail-closed exact match against `bundle.browser.domNodes[]` (Rule 3 / C-080); tested (`dom-node.test.js`) and calibrated (`p4-verifier-fabricated-domnode.json`, wired into `eval/calibration-known-bad/run.js` as `breach-domnode-artifact-rejected`), reached via `index.js`'s dispatch, but no `mint/` entry point calls it yet | Aman | same as above |
...
| `breach/adjudicator/adjudicate.js` | the adjudicator orchestrator (`adjudicate(candidates, bundle, opts)`; filter-only per Rule 11); a `breach` verdict is gated through Rule 12 gate 3 (`llm/entailment.js` NLI) before it ships as a `violation`, a non-entailment demotes to needs_review; tested (`adjudicate.test.js`) and calibrated (`p3-adjudicator-invented-finding.js`), but no `mint/` entry point calls it yet | Aman | `mint/` requires it for the propose-verify-adjudicate pipeline |
